...
class ExcelProcessor:
    ...
    def ler_contratos_pendentes(limite=None, modo_teste=False):
        try:
            ...
            print(f"\n📂 Lendo planilha de contratos: {caminho_excel}")
            ...
            if modo_teste:
                ...
                print("   ⚠️ Modo de teste ativado → processando apenas 1 contrato")
    ...
    def atualizar_esteira(resultados, caminho_excel):
        try:
            ...
            if not os.path.exists(caminho_excel):
                df_novos.to_excel(caminho_excel, engine='openpyxl', sheet_name='#RPA', index=False)
                print(f"✅ Planilha criada em: {caminho_excel}")
                return True
            ...
            try:
                ...
                print(f"✅ Planilha atualizada → {len(df_novos)} contratos adicionados")
                return True
                ...
                df_novos[colunas].to_excel(caminho_excel, engine='openpyxl', sheet_name='#RPA', index=False)
                print(f"⚠️ Nova aba criada pois a leitura da planilha falhou")
                return True
            ...
            if "File is not a zip file" in str(e):
                ...
                pd.DataFrame({'Contrato': [], 'Status': []}).to_excel(
                    caminho_excel, engine='openpyxl', sheet_name='#RPA', index=False
                )
                print("⚠️ Planilha corrompida foi recriada do zero")
            return False

    @staticmethod
    def preencher_planilha(dados, linha_inicial=7):
        try:
            pluxxe_path = 'modelo.xlsx'
    ...
    def renomear_saida():
        ...
        print("🖼️ Logo adicionada à planilha")
    ...
    def compilar_arquivos():
        ...
        print(f"📂 Arquivos compilados: {arquivos_processados}")
